...
void MophoLookAndFeel::fillTextEditorBackground(Graphics& g, int /*w*/, int /*h*/, TextEditor& textEditor) {
	auto editorIsInImportExportBrowser{ textEditor.getParentComponent()->getComponentID() == ID::comp_ImportExportBrowser.toString() };
	auto editorIsInPathComboBoxInImportExportBrowser{ 
		textEditor.getParentComponent()->getParentComponent()->getParentComponent()->getComponentID() == ID::comp_ImportExportBrowser.toString()
	};
...
TextLayout MophoLookAndFeel::layoutTooltipText(const String& text, Colour colour) noexcept {
	...
	s.append(text, GUI::font_Tooltips, colour);
	TextLayout tl;
	const float tooltipMaxWidth{ 400.0f };
	tl.createLayout(s, tooltipMaxWidth);
	return tl;
}
...
void MophoLookAndFeel::positionComboBoxText(ComboBox& box, Label& label) {
	...
	if (parentComponent != nullptr) {
		auto isInImptExptFileBrowser{ parentComponent->getComponentID() == ID::comp_ImportExportBrowser.toString() };
