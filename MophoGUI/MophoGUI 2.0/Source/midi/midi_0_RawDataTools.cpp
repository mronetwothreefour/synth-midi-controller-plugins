    ...
    const uint8* dumpData, ExposedParameters* exposedParams, UnexposedParameters* unexposedParams)
{
    ...
    for (uint8 param = 0; param != EP::numberOfExposedParams; ++param) {
        auto paramID{ exposedParams->info.IDfor(param) };
        auto lsByteLocation{ exposedParams->info.lsByteLocationFor(param) };
        auto msBitLocation{ exposedParams->info.msBitPackedByteLocationFor(param) };
        auto msBitMask{ exposedParams->info.msBitMaskFor(param) };
...
const std::vector<uint8> RawDataTools::extractRawDataFromExposedParameters(ExposedParameters* exposedParams) {
    ...
    for (uint8 paramIndex = 0; paramIndex != EP::numberOfExposedParams; ++paramIndex) {
        ...
        auto msbLocation{ exposedParams->info.msBitPackedByteLocationFor(paramIndex) };
        auto lsbLocation{ exposedParams->info.lsByteLocationFor(paramIndex) };
        if (paramValue > 127) {
            voiceData[msbLocation] += exposedParams->info.msBitMaskFor(paramIndex);
        }
