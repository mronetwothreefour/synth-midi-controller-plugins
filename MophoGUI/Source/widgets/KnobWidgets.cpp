...
String KnobWidget_OscPitch::createTooltipString(const int& currentValue) const noexcept
{
	...
	if (privateParams->shouldShowValueTip())
		tooltip += "Current Value: " + convertIntToValueString(MophoParameterType::oscPitch, currentValue, true) + "\n" ;
...
String KnobWidget_LFOfreq::createTooltipString(const int& currentValue) const noexcept
{
	...
	if (privateParams->shouldShowInfoTip())
	{
		...
		tooltip += "At 0, 1 cycle lasts 30 sec. At 89, the frequency is almost 8 Hz.\n";
		tooltip += "From 90 to 150, the LFO has a pitched frequency, increasing\n";
		tooltip += "in semitones steps from C 0 (8 Hz) up to C 5 (261 Hz).\n";
