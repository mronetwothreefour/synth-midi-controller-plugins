...
Look_And_Feel_G::Look_And_Feel_G(float& scale_factor) :
...
void Look_And_Feel_G::getIdealPopupMenuItemSizeWithOptions(const String& /*txt*/,
...
void Look_And_Feel_G::drawLabel(Graphics& g, Label& lbl) {
	...
	if (id == ID::label_slider || id == ID::label_cbox) {
...
void Look_And_Feel_G::fillTextEditorBackground(Graphics& g, int /*w*/, int /*h*/,
											   TextEditor& editor)
{
	if (editor.getComponentID() == ID::txt_editor_filled)
		g.fillAll(findColour(TextEditor::backgroundColourId));
}

void Look_And_Feel_G::drawToggleButton(Graphics& g, ToggleButton& btn, bool hilited, bool down) {
	drawTickBox(g, btn, 0.0f, 0.0f, (float)btn.getWidth(), (float)btn.getHeight(),
				btn.getToggleState(), btn.isEnabled(), hilited, down);
}

void Look_And_Feel_G::drawTooltip(Graphics& g, const String& txt, int w, int h) {
...
Rectangle<int> Look_And_Feel_G::getTooltipBounds(const String& txt, Point<int> pos,
...
TextLayout Look_And_Feel_G::layout_tip_text(const String& txt) noexcept {
