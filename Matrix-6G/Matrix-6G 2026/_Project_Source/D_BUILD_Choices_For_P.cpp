...
const StringArray Choices_For::input_source(const bool for_lfo, const bool for_mmod) {
	StringArray list;
	list.add("NONE__none");
	list.add(std::string{ for_lfo ? "ENV1" : "ENV 1" } + "__envelope 1");
	list.add(std::string{ for_lfo ? "ENV2" : "ENV 2" } + "__envelope 2");
	list.add(std::string{ for_lfo ? "ENV3" : "ENV 3" } + "__envelope 3");
	list.add(std::string{ for_lfo ? "LFO1" : "LFO 1" } + "__LFO 1");
	list.add(std::string{ for_lfo ? "LFO2" : "LFO 2" } + "__LFO 2");
	list.add(std::string{ for_lfo ? "VIB" : "VIBRATO" } + "__vibrato LFO");
	list.add(std::string{ for_lfo ? "RAMP1" : "RAMP 1" } + "__ramp generator 1");
	list.add(std::string{ for_lfo ? "RAMP2" : "RAMP 2" } + "__ramp generator 2");
	list.add(std::string{ for_lfo ? "KEYB" : for_mmod ? "KEYBD" : "KEYBOARD" } + "__keyboard scaling");
	list.add(std::string{ for_lfo ? "PORT" : "PORTA" } + "__portamento speed");
	list.add(std::string{ for_lfo ? "TRAK" : for_mmod ? "TRACK" : "TRACKING" } + "__tracking generator");
	list.add("GATE__keyboard note gate");
	list.add(std::string{ for_lfo ? "VEL" : for_mmod ? "VELO" : "VELOCITY" } + "__keyboard note velocity");
	list.add(std::string{ for_lfo ? "RVEL" : for_mmod ? "R VELO" : "REL VELO" } + "__keyboard release velocity");
	list.add(std::string{ for_lfo ? "PRES" : for_mmod ? "PRESS" : "PRESSURE" } + "__keyboard pressure (aftertouch)");
	list.add(std::string{ for_lfo ? "PED1" : "PEDAL 1" } + "__pedal 1");
	list.add(std::string{ for_lfo ? "PED2" : "PEDAL 2" } + "__pedal 2");
	list.add(std::string{ for_lfo ? "LEV1" : "LEVER 1" } + "__lever 1 ");
	list.add(std::string{ for_lfo ? "LEV2" : "LEVER 2" } + "__lever 2");
	list.add(std::string{ for_lfo ? "LEV3" : "LEVER 3" } + "__lever 3 (lever 2 reverse)");
	return list;
}
