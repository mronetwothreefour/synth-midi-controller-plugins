...
public:
	...
	void paint(Graphics& g) override
	{
		if (knob != nullptr)
		{
			...
			if (knobValue == (int)OscWaveShape::off) { paintText(g, "OFF"); }
			if (knobValue == (int)OscWaveShape::sawtooth) { paintSawtooth(g, path, strokeType); }
			if (knobValue == (int)OscWaveShape::triangle) { paintTriangle(g, path, strokeType); }
			if (knobValue == (int)OscWaveShape::sawTriMix) { paintSawTriMix(g, path, strokeType); }
			if (knobValue >= (int)OscWaveShape::pulse) { paintPulse(g, path, strokeType, knobValue - (int)OscWaveShape::pulse); }
		}
		else { paintText(g, "ERR"); }
	}
	...
	void sliderValueChanged(Slider* slider) override { if (slider == knob) repaint(); }
};
