...
void Draw_Paths_Main_P::backdrop(Graphics& g) {
	...
	g.fillPath(load_path(PATH::main_translucent_white));
	g.setColour(COLOR::grey_dark);
	g.fillPath(load_path(PATH::main_grey_dark));
	g.setColour(COLOR::red_led_1);
	g.fillPath(load_path(PATH::main_red_led_1));
	g.setColour(COLOR::red_led_2);
	g.fillPath(load_path(PATH::main_red_led_2));
	g.setColour(COLOR::red_btn);
	g.fillPath(load_path(PATH::main_red_btn));
	g.setColour(COLOR::grey_lite);
	g.fillPath(load_path(PATH::main_grey_lite));
