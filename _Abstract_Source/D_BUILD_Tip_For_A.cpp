...
std::string Tip_For_A::from_string_literal(const char8_t* sl) {
    String s{ sl };
...
{
    String s{ from_string_literal(SL::edit_pitch_tip) };
...
std::string Tip_For_A::txt_editor_s_int(const int limit) {
    String s{ from_string_literal(SL::edit_s_int_tip) };
    s = s.replace("_", (String)abs(limit));
    return s.toStdString();
}

std::string Tip_For_A::txt_editor_u_int(const unsigned int max) {
    String s{ from_string_literal(SL::edit_u_int_tip) };
