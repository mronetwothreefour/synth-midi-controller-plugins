...
void Slider_Label::on_editor_show() {
	...
	if (n == NAME::lbl_lfo_freq)
		edit->setInputRestrictions(5, allowed_chars_lfo_freq);
	...
	if (n == NAME::lbl_osc_shape)
		edit->setInputRestrictions(4, allowed_chars_osc_shape);
	if (n == NAME::lbl_osc_slop)
		edit->setInputRestrictions(1, allowed_chars_osc_slop);
	if (n == NAME::lbl_seq_step) {
		edit->setInputRestrictions(4, allowed_chars_seq_step);
		edit->applyFontToAllText(Font_For::seq_step(scale_factor));
	}
	if (n == NAME::lbl_seq_step_trk_1) {
		edit->setInputRestrictions(4, allowed_chars_seq_step_trk_1);
		edit->applyFontToAllText(Font_For::seq_step(scale_factor));
	}
...
void Slider_Label::set_text_to_stored_choice() {
	...
	auto n = getName();
	if (n == NAME::lbl_lfo_freq)
		parent_slider->for_pitch = choice_num >= lfo_1st_pitch_freq && choice_num < 151;
