	...
	customVoiceNameStrings{ ID::banks_CustomVoiceNameStrings }
{
	for (auto bankNum = 0; bankNum != 3; ++bankNum) {
		ValueTree factoryNamesChildTree{ (String)bankNum };
		ValueTree customDataChildTree{ (String)bankNum };
		ValueTree customNamesChildTree{ (String)bankNum };
...
const String VoicesBanks::nameOfVoiceInBankSlot(const VoicesBank bank, const uint8 slotNum) const {
	...
	auto bankName{ String((int)bank % 3) };
	...
	return bankTree.getProperty(String(slotNum).paddedLeft('0', 3));
}
...
void VoicesBanks::valueTreePropertyChanged(ValueTree& tree, const Identifier& propertyID) {
	auto slotNum{ (uint8)propertyID.toString().getIntValue() };
	auto bankNum{ tree.getType().toString().getIntValue() };
